...
- 图片类型选择
  ...
  - CSS 雪碧图
    ...
    > 优点：减少 HTTP 请求
    > 缺点：合成的图片可能会很大，如果合成图片加载失败，造成的影响比较大
    >
...
- 重排与重绘
  ...
  当浏览器进行重排与重绘时，会导致 UI 频繁渲染，从而进一步阻塞 JS。
  所以 **注意 CSS 的性能也是提高网页访问速度的重要因素**。编写 CSS 代码时，要尽量减少 UI 的重绘与重排。

  **概念介绍：**

  - 重排

    当页面中一些元素的尺寸，布局，隐藏等改变，需要重新构建 render tree。这就成为重排。

    **会触发重排的属性：**

    ![css_backflow](./images/css_backflow.png)
  ...
  - 重绘
    ...
    > 重排一定引起重绘，重绘不一定引起重排
    >
    > 通过添加合成层（并不是 `z-index` 定位的层），让频繁重排和重绘的元素单独在一个合成层，以此来减少重排和重绘对整个页面的影响。但是合成层过多又会增加浏览器进行合成层合成的时间。所以两者要进行权衡。
    >
    > 改变元素合成层的方式：
    >
  ...
  **flush 队列：**
  ...
  > 其实浏览器自身是有优化策略的。浏览器会维护一个队列，把所有引起重排、重绘的操作放入这个队列，等队列中的操作达到一定数量或到了一定时间间隔，浏览器就会 flush 这个队列，进行一个批处理。这样就让多次重排和重绘变成了一次重排重绘。
  ...
  **一些 CSS 属性优化点：**
  ...
  - 对于那些会强制刷新浏览器 flush 队列的 CSS 属性，不要放在循环里。这些 CSS 属性如下：
    ...
    > 使用这些属性时，浏览器会强制刷新队列，使得重排重绘每次都要进行

  - 尽量不用 table 布局

    > 如果使用 table 布局，只要 table 中有一个部分改变，就会引起整个 table 的重排或重绘
...
- 减少页面绘制时的重排和重绘
- CSS 属性的书写顺序
- 尽量避免使用“引起重排的属性”（如 transfrom 代替 left, top 等）
