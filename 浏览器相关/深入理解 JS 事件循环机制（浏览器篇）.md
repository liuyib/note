...
5. [async 函数返回值](https://juejin.cn/post/7194744938276323384#heading-1)
   ...
   2. 返回值：thenable（等待 1 个微任务的时间，然后放行，继续**下面**的代码）
   3. 返回值：`Promise` 类型（等待 2 个微任务的时间，然后放行，继续**下面**的代码）
...
6. [await 右值类型区别](https://juejin.cn/post/7194744938276323384#heading-2)
   ...
   1. 接非 thenable 类型：AAA 不等待
   2. 接 thenable 类型：AAA 等待 1 个微任务的时间
   3. 接**状态确定** `Promise` 类型：AAA 不等待
